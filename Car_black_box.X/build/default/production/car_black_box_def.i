...
void display_dash_board(char event[], unsigned char speed[])
{
      ...
      clcd_print(event[count],(0xC0 + 10));
      _delay((unsigned long)((100)*(20000000/4000.0)));

      adc_val=read_adc(0x00)/10.23;
...
void view_log(unsigned char key, unsigned char reset_flag) {
    ...
    if (event_count == 0) {
        ...
        if (key == 0x2F && rpos < (event_count - 1)) {
            rpos++;
        } else if (key == 0x37 && rpos > 0) {
            rpos--;
        }
        if(rpos>9)
        {

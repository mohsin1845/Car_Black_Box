...
unsigned char control_flag=0x02;
...
void main(void)
{
    ...
    while (1)
    {
        ...
        else if (key == 0x3D && (count >=0 && count<6))
        {
            count++;
            store_event(count);
        }
        else if (key == 0x3B && (count > 1 && count != 7))
        {
            count--;
            store_event(count);
        }

        else if ((control_flag==0x02) &&(key==0x37 || key==0x2F))
        {
            clear_screen();
        ...
        else if(control_flag==0x06 && (key==0x1F))
        {
            switch(menu_pos)
            {
                case 0:
        ...
        switch (control_flag)
        {
            ...
            case 0X0C:
            {
                ...
                for (int k = 0; k < event_count; k++)
                {
                    ...
                    for (i = 0; i < 14; i++) {
                        if (i == 2 || i == 5) {
                            ...
                            putchar(read_ext_eeprom(k * 10 + j));
                            j++;
                        }
                    }
                    puts("\n\r");
                ...
                clear_screen();
                clcd_print("Files Downloaded", (0xC0 + 0));
                for (long delay = 500000; delay--;);
                clear_screen();
                TXIE = 0;
       RCIE = 0;
                control_flag = 0x06;
